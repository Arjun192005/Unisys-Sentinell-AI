...
class PromptLog(models.Model):
    ...
    semantic_flags = models.JSONField(default=list, blank=True)       # intent categories detected
    semantic_confidence = models.FloatField(default=0.0)              # 0.0–1.0
    attack_vectors = models.JSONField(default=list, blank=True)       # specific attack pattern names

    # ── Layer 3: Provenance / Trust Fields ────────────────────────────────
    trust_score = models.FloatField(default=1.0)                      # 0.0–1.0
    trust_level = models.CharField(max_length=20, default="TRUSTED")  # TRUSTED/SEMI_TRUSTED/UNTRUSTED
    source_type = models.CharField(max_length=20, default="TEXT")     # TEXT/FILE/PASTED/RETRIEVED
    anomaly_flags = models.JSONField(default=list, blank=True)        # behavioral anomalies
